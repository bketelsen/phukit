...
This document describes the Incus-based integration testing for phukit.
...
Verifies `phukit list` can discover available disks in the VM.

### 2. Validate Disk

Tests `phukit validate` correctly identifies suitable installation targets.
...
```bash
incus info phukit-test-<PID> --show-log
```
...
- `/tmp/phukit-install-<PID>.log` - Installation log
- `/tmp/phukit-update-<PID>.log` - Update log
...
# Delete stuck VM
incus delete phukit-test-<PID> --force

# List and delete storage volumes
incus storage volume list default
incus storage volume delete default phukit-test-<PID>-disk
```
...
```bash
incus exec phukit-test-<PID> -- bash
```
...
Incus tests provide the highest confidence that phukit works in real-world scenarios.
